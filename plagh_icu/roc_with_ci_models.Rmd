...
library(ggplot2)
library(readr)
extval_probability <- read_csv('../results/PLAGH_posibility.csv')
```
...
auc_ci_df
write.csv(auc_ci_df, 'auc_ci_PLAGH_MDRO_models.csv')
```
...
for(i in 1:7) {
  ...
  p <- p + theme(legend.justification=c(1,0),
    ...
    scale_colour_manual(
                  ...
                  labels=c("lr  AUC: 0.740","knn  AUC: 0.697","svc  AUC: 0.699", "rf  AUC: 0.784", "xgb  AUC: 0.780", "mlp  AUC: 0.748","ensemble  AUC: 0.786"))
} 
p
```
...
```{r}
pdf("ROC_PLAGH_MDRO_models.pdf", bg="white",width=5,height=5)
p
dev.off()
```
...
calibration_plot <- valProbggplot(
  ...
  CL.smooth = TRUE, 
  logistic.cal = TRUE,
  lty.log = 2, col.log = "green", lwd.log = 1.5,xlim = c(0, 0.6), ylim = c(0, 1),
  dostats = FALSE
)
calibration_plot
```
...
geom_segment(aes(x = 0, y = 0, xend = 0.6, yend = 0.6), color = "lightblue", size = 1)
# 打印定制后的图形
print(ggplot_obj)
...
```{r}
pdf("Cali_PLAGH_MDRO_models.pdf", bg="white",width=8,height=6)
ggplot_obj
dev.off()
```
...
pdf("DCA_PLAGH_MDRO_models.pdf", bg="white",width=6,height=5)
